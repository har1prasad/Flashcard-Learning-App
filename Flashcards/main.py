...
DATA_FILE = "data/Sanskrit.csv" #for changing the data file
...
def next_card():
    ...
    if not data_dictionary:  # Check if there are no words left
        ...
        canvas.itemconfig(canvas_img, image=card_front)
        canvas.itemconfig(card_title, text=f"{data.columns[0]}", fill="black")
        canvas.itemconfig(card_word, text=current_card[f"{data.columns[0]}"], fill="black")
        flip_timer = window.after(3000, func=flipping)


def flipping():
    canvas.itemconfig(canvas_img, image=card_back)
    canvas.itemconfig(card_title, text=f"{data.columns[1]}", fill="white")
    canvas.itemconfig(card_word, text=current_card[f"{data.columns[1]}"], fill="white")
